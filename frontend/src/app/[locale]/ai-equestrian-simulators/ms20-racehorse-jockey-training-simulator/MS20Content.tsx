...
export default function MS20Content({ product, locale }: MS20ContentProps) {
  ...
  return (
    <>
      <style>{`
        .feature-cards-container::-webkit-scrollbar { display: none; }
      `}</style>
      ...
      <section style={{ position: 'relative', background: '#0A1E3F', minHeight: '600px' }}>
        ...
        <div id="ms20-hero-grid" style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            ...
            <img
              src="/images/ms20.png"
              alt="MS.20 jockey training racehorse speed simulator"
              style={{
      ...
      {/* Immersive Section */}
      <section id="ms20-immersive-section" style={{ position: 'relative', height: '500px', overflow: 'hidden' }}>
        <img src="/images/ms20-immersive.jpg"
          alt="MS.20 jockey training simulator immersive racing experience"
        ...
        <div style={{ position: 'relative' }}>
          {scrollLeft > 0 && (
            <button onClick={() => scrollRef.current?.scrollBy({ left: -370, behavior: 'smooth' })}
              style={{ position: 'absolute', left: '-18px', top: '50%', transform: 'translateY(-50%)', zIndex: 10, width: '36px', height: '36px', borderRadius: '50%', border: '1px solid #E0D8C8', background: '#fff', cursor: 'pointer', fontSize: '18px', color: '#C9A84C', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>‹</button>
          )}
          {canScrollRight && (
            <button onClick={() => scrollRef.current?.scrollBy({ left: 370, behavior: 'smooth' })}
              style={{ position: 'absolute', right: '-18px', top: '50%', transform: 'translateY(-50%)', zIndex: 10, width: '36px', height: '36px', borderRadius: '50%', border: '1px solid #E0D8C8', background: '#fff', cursor: 'pointer', fontSize: '18px', color: '#C9A84C', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>›</button>
          )}
          ...
          <div ref={scrollRef}
            ...
            style={{ display: 'flex', flexDirection: 'row', gap: '24px', overflowX: 'auto', paddingBottom: '16px', paddingLeft: '4px', paddingRight: '24px', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}
            className="feature-cards-container">

            {/* Card 1 */}
            <div style={{ flex: '0 0 calc(33.333% - 16px)', minWidth: '280px', scrollSnapAlign: 'start', background: '#ffffff', border: '1px solid #E0D8C8', borderRadius: '20px', padding: '40px 32px', transition: 'all 0.3s' }}
              ...
              <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#0A1E3F', marginBottom: '12px' }}>{t('features.card1.title')}</h3>
              <p style={{ fontSize: '14px', color: '#5A6478', lineHeight: 1.6, marginBottom: '20px' }}>{t('features.card1.desc')}</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {card1Tags.map((tag, i) => <span key={i} style={{ fontSize: '11px', fontWeight: 600, padding: '4px 10px', borderRadius: '12px', color: '#0A1E3F', background: 'rgba(201,168,76,0.12)', border: '1px solid rgba(201,168,76,0.35)' }}>{tag}</span>)}
              </div>
            </div>

            {/* Card 2 - Highlighted */}
            <div style={{ flex: '0 0 calc(33.333% - 16px)', minWidth: '280px', scrollSnapAlign: 'start', backgroundImage: "url('/images/ms20-gallop.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid #C9A84C', borderTop: '3px solid #C9A84C', borderRadius: '20px', padding: '40px 32px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#ffffff', marginBottom: '12px' }}>{t('features.card2.title')}</h3>
                <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', lineHeight: 1.8, marginBottom: '20px' }}>{t('features.card2.desc')}</p>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {card2Tags.map((tag, i) => <span key={i} style={{ padding: '4px 10px', background: 'rgba(201,168,76,0.2)', borderRadius: '12px', fontSize: '11px', fontWeight: 600, color: '#C9A84C', border: '1px solid rgba(201,168,76,0.5)' }}>{tag}</span>)}
                </div>
              </div>
            </div>
            ...
            <div style={{ flex: '0 0 calc(33.333% - 16px)', minWidth: '280px', scrollSnapAlign: 'start', background: '#ffffff', border: '1px solid #E0D8C8', borderRadius: '20px', padding: '40px 32px', transition: 'all 0.3s' }}
              ...
              onMouseLeave={e => { e.currentTarget.style.background = '#ffffff'; e.currentTarget.style.borderColor = '#E0D8C8'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; }}>
              <div style={{ fontSize: '48px', marginBottom: '16px' }}>🥇</div>
              <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#0A1E3F', marginBottom: '12px' }}>{t('features.card3.title')}</h3>
              <p style={{ fontSize: '14px', color: '#5A6478', lineHeight: 1.6, marginBottom: '20px' }}>{t('features.card3.desc')}</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {card3Tags.map((tag, i) => <span key={i} style={{ fontSize: '11px', fontWeight: 600, padding: '4px 10px', borderRadius: '12px', color: '#0A1E3F', background: 'rgba(201,168,76,0.12)', border: '1px solid rgba(201,168,76,0.35)' }}>{tag}</span>)}
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '80px 6%', background: '#F5F0E8', borderTop: '1px solid #E0D8C8', textAlign: 'center' }}>
        <h2 style={{ fontSize: '32px', fontWeight: 700, color: '#0A1E3F', marginBottom: '40px' }}>{t('video.title')}</h2>
        {product.video_file && (
          <video
      ...
      <section style={{ padding: '80px 0', background: '#F5F0E8', borderTop: '1px solid #E0D8C8', width: '100%' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 60px' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ fontSize: '32px', fontWeight: 700, color: '#0A1E3F', marginBottom: '14px' }}>{t('specs.title')}</h2>
      ...
      <section style={{ padding: '72px 6%', background: '#F9F7F4' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '48px', marginTop: '36px', flexWrap: 'wrap' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'clamp(28px, 3vw, 42px)', fontWeight: 800, color: '#C9A84C' }}>200KG</div>
              <div style={{ fontSize: '13px', color: '#5A6478', marginTop: '4px' }}>{t('whatIs.stat1')}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'clamp(28px, 3vw, 42px)', fontWeight: 800, color: '#C9A84C' }}>113s</div>
              <div style={{ fontSize: '13px', color: '#5A6478', marginTop: '4px' }}>{t('whatIs.stat2')}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'clamp(28px, 3vw, 42px)', fontWeight: 800, color: '#C9A84C' }}>6+</div>
      ...
      <section style={{ padding: '80px 6%', background: '#0A1E3F', textAlign: 'center' }}>
        <h2 style={{ fontSize: '32px', fontWeight: 700, color: '#fff', marginBottom: '16px' }}>{t('cta.title')}</h2>
        <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.8)', marginBottom: '40px', maxWidth: '600px', margin: '0 auto 40px auto' }}>
          {t('cta.subtitle')}
        </p>
        <div id="ms20-cta-buttons" style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
